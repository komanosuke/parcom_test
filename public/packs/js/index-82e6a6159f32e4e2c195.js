...
function graph() {
  ...
  var $array = document.getElementById('title').textContent;
  $array = String($array);
  $array = $array.replace("[", "");
  $array = $array.replace("]", "");
  $array = $array.split(",");
...
function postSettingData() {
  var fan_data = document.getElementsByName('FAN');
  for (var i = 0; i < fan_data.length; i++) {
    if (fan_data.item(i).checked) {
      var _checkValue = fan_data.item(i).value;
      console.log("選択されているのは" + _checkValue + "です");
    }
  }
...
/******/ });
//# sourceMappingURL=index-82e6a6159f32e4e2c195.js.map